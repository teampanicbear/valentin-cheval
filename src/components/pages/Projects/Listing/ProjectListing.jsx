...
const ProjectListing = (props) => {
    ...
    onMount(() => {
        ...
        let ignoreElement = window.innerWidth <= 767 ? ['.project__name .origin-wrap .project__name-txt'] : [];
        elements.forEach((el) => {
    ...
    const pageTransition = () => {
        ...
        tl
            .fromTo(transitionDOM('name'),
                ...
                {
                    y: window.innerWidth > 767 ? getBoundingTransition('name').to.top : (getBoundingTransition('name').to.top - transitionDOM('name').offsetTop),
                    scale:
    ...
    const animationBackInit = (initIndex) => {
        let ignoreElement = window.innerWidth > 991 ? ['.project__desc-txt'] : ['.project__role-listing', '.project__name .origin-wrap .project__name-txt', '.project__services-listing', '.project__selling-listing', '.project__desc-txt', '.project__year-txt', '.home__project-pagination-txt'];
        ...
        let leftOffset = document.querySelectorAll('.project__name-wrap .origin-wrap .project__name-txt')[initIndex].offsetLeft - parseFloat(getComputedStyle(document.querySelector('.container')).paddingLeft)
    ...
    const animationText = (nextValue, direction, isInit) => {
        ...
        if (window.innerWidth <= 767) {
            if (isInit) {
                gsap.set('.project__name-wrap', { x: -document.querySelector('.project__name-wrap .origin-wrap').offsetWidth - cvUnit(32, 'rem') })
            }
            else {
                if (_direction < 0 && nextValue === props.data.length - 1) {
                    gsap.to('.project__name-wrap', {
                        x: -document.querySelectorAll('.project__name-wrap .clone-wrap')[0].querySelectorAll('.project__name-txt')[props.data.length - 1].offsetLeft,
                        duration: 1.1,
                        ease: 'power3.inOut',
                        onComplete: () => {
                            gsap.set('.project__name-wrap', { x: -document.querySelector('.project__name-wrap .origin-wrap').querySelectorAll('.project__name-txt')[props.data.length - 1].offsetLeft });
                        }
                    });
                }
                    ...
                    gsap.to('.project__name-wrap', {
                        ...
                        ease: 'power3.inOut',
                        onComplete: () => {
                            gsap.set('.project__name-wrap', { x: -document.querySelector('.project__name-wrap .origin-wrap').offsetLeft });
                        }
                    });
                }
                else {
                        gsap.to('.project__name-wrap', {
                            x: -document.querySelectorAll('.project__name-wrap .origin-wrap .project__name-txt')[nextValue].offsetLeft, duration: 1.1, ease: 'power3.inOut'
                        });
                    }
                }
        }
