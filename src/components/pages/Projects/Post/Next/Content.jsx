...
function Content(props) {
  const pageTransition = () => {
    ...
    document.querySelector('.sc__next-post.fake .next__post-img-inner').innerHTML =
      document.querySelector('.sc__next-post.default .next__post-img-inner').innerHTML;
      ...
      gsap
        .timeline({
          ...
          onComplete() {
            ...
            document
              .querySelector('.sc__next-post.fake .next__post-title .current')
              .removeAttribute('style');
          },
        })
        .set('.sc__next-post.fake', { zIndex: 5, duration: 0 })
        .fromTo(
          '.sc__next-post.fake .next__post',
          { top: rect.top, height: rect.height },
          { top: 0, height: window.innerHeight },
          '<=0'
        )
        .fromTo(
          '.sc__next-post.fake .next__post-img-inner',
          {
            opacity: window.innerWidth > 991 ? 0.7 : 0.3,
            scale: window.innerWidth > 991 ? 1.2 : 1,
          },
          { opacity: 1, scale: 1 },
          '<=0'
        )
        .fromTo(
          title.from,
          { y: 0 },
          { y: -(title.from.getBoundingClientRect().top - title.to.getBoundingClientRect().top) },
          '<=0'
        )
        .to('.sc__next-post.fake', {
          autoAlpha: 0,
          ease: 'linear',
          duration: 0.4,
          clearProps: 'all',
