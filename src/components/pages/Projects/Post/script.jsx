...
const PostScript = (props) => {
    ...
    const setUpPageTransition = (data) => {
        if (document.querySelector('.project__transition').classList.contains('can-return')) {
            ...
            gsap.set(transitionDOM('name'), {
                y: document.querySelector('.post__hero-title').getBoundingClientRect().top - (window.innerWidth <= 767 ? transitionDOM('name').offsetTop : 0),
                scale:
    ...
    onMount(() => {
        ...
        if (window.innerWidth <= 767) {
            ...
            gsap
                .timeline({ delay: .6 })
                .to('.post__hero-cover-gradient', { autoAlpha: 1, duration: .5 })
                .to(title.words, {
                    autoAlpha: 1, yPercent: 0, duration: .8, clearProps: 'all', onComplete: () => {
                        title.revert();
                        gsap.set('.post__hero-title', { clearProps: 'all' });
                    }
                })
                .to(desc.words, {
                    autoAlpha: 1, yPercent: 0, duration: .8, stagger: .01, onComplete: () => {
                        desc.revert();
                        gsap.set('.post__hero-desc', { clearProps: 'all' });
                    }
                }, "<=0.2")
                .to([roleLabel.words, roleListing.words], {
                    autoAlpha: 1, yPercent: 0, duration: .8, stagger: .01, onComplete: () => {
                        roleLabel.revert(); roleListing.revert();
                        gsap.set('.post__hero-role .post__hero-label, .post__hero-role .post__hero-listing-item', { clearProps: 'all' });
                    }
                }, "<=0.3")
                .to([serviceLabel.words, serviceListing.words], {
                    autoAlpha: 1, yPercent: 0, duration: .8, stagger: .01, onComplete: () => {
                        serviceLabel.revert(); serviceListing.revert();
                        gsap.set('.post__hero-service .post__hero-label, .post__hero-service .post__hero-listing-item', { clearProps: 'all' });
                    }
                }, "<=0.3")
                .to([sellingLabel.words, sellingListing.words], {
                  autoAlpha: 1, yPercent: 0, duration: .8, stagger: .01, onComplete: () => {
                      sellingLabel.revert(); sellingListing.revert();
                      gsap.set('.post__hero-selling .post__hero-label, .post__hero-selling .post__hero-listing-item', { clearProps: 'all' });
                  }
              }, "<=0.3")
        }
